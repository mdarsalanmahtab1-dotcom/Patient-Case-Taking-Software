...
# 1. Create Patient
try:
    req_pat = urllib.request.Request('http://127.0.0.1:8000/api/patient', data=json.dumps({"name": "Test", "phone": "12345"}).encode('utf-8'), headers={'Content-Type': 'application/json'})
    res_pat = urllib.request.urlopen(req_pat)
    pat_id = json.loads(res_pat.read().decode())["patient_id"]
    print("Patient ID:", pat_id)

    # 2. Start session
    req_start = urllib.request.Request('http://127.0.0.1:8000/api/session/start', data=json.dumps({"patient_id": pat_id}).encode('utf-8'), headers={'Content-Type': 'application/json'})
